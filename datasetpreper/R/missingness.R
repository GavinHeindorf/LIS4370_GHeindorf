#' Missingness
#'
#' @description
#' 'missingness()' provides one the opportunity to very easily
#' summarize the amount of missing values in their data and to
#' conveniently remove variables or observations that are not
#' full cases.
#'
#'
#' @param data A dataframe or vector which is to be checked for missing data.
#' @param missing A single value or special character (default = NA)
#' that is to be treated as a missing value (e.g., 0, NULL, 99).
#' @param returnFulls T/F, would you like only full instances, those with
#' no missing data, to be returned?
#' @param histogram T/F, should a histogram illustrating the amount of
#' missing data per observation or variable be printed? (only an option
#' if 'data' is a dataframe or matrix)
#' @param axis 1 = determine missingness by row; 2 = determine
#' missingness by column. (only necessary if 'data' is a dataframe or
#' matrix)
#'
#' @return 'missingness()' returns a numerical value equal to the
#' number of missing data points if 'data' is a vector. If 'data' is a
#' matrix or dataframe, a vector equal to the number of rows (axis = 1) or
#' columns (axis = 2) is returned with the number of missing values for
#' each instance or variable. If 'returnFulls' is True, then a list is
#' returned with 'missingness' containing the vector of counts for each
#' instance or variable, and 'newdata'-consisting of all instances or
#' variables from the original dataset that had no missing values.
#'
#' @examples
#'
#' missingness(datapreper::ex_QuestData$Q2,
#'             missing = NA,
#'             returnFulls = FALSE)
#'
#' missingness(datapreper::ex_QuestData,
#'             missing = NA,
#'             histogram = TRUE,
#'             axis = 2)
#'
#' clean_quest_data <- missingness(datapreper::ex_QuestData,
#'                                 missing = NA,
#'                                 returnFulls = TRUE,
#'                                 histogram = FALSE,
#'                                 axis = 1)
#'
#'
#' @import ggplot2
#' @export

missingness <- function(data, missing = NA,
                        returnFulls = FALSE,
                        histogram = FALSE,
                        axis = 1){

  if (is.vector(data)){
    returnFulls = F
    if (histogram){
      warning("Ignorning histogram = T because 'data' is vector")
      histogram = F
    }
  } else if (is.data.frame(data) | is.matrix(data)){
    stopifnot(axis %in% c(1, 2))
  } else{
    stop("ERROR: 'data' is not a dataframe, matrix, or vector")
    }

  stopifnot(is.logical(histogram), is.logical(returnFulls))


  if (is.vector(data)){
    if (is.na(missing)){
      return(sum(is.na(data)))
    } else {
      return(length(which(data == missing)))
    }
  } else{

    if (is.na(missing)){
      res <- apply(t(apply(data, axis, is.na)), axis, sum)
    } else {
      counter <- function(x, miss){return(length(which(x == miss)))}
      res <- apply(data, axis, counter, miss = missing)
    }

    if (returnFulls){
      fulls <- which(res == 0)
      if (axis == 1){
        toreturn <- list(newdata = data[fulls,], missingness = res)
      } else {
        toreturn <- list(newdata = data[fulls], missingness = res)
      }
    } else {
      toreturn <- res
    }

    if (histogram){

      posbins <- length(unique(res))
      if (posbins > 30){
        posbins <- 30
      }

      print(ggplot2::ggplot(data.frame(xy = res), aes(x = xy))+
              geom_histogram(bins = posbins, color = "red", fill = "grey57")+
              theme_classic()+
              labs(x = "Number of Missing Values", y = "Frequency",
                  title = "Distribution of Missing Values within Dataframe"))
    }

    return(toreturn)
  }
}
